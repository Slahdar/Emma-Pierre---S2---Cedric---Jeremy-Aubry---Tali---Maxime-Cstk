...
function handleFilterChange() {
    ...
    const selectedGemTypes = Array.from(document.querySelectorAll('input[name="gemTypes[]"]:checked'))
        .map(checkbox => checkbox.value);
    ...
    const filteredProducts = allProducts.filter(product => {
        return (
            (!selectedCollections.length || selectedCollections.includes(String(product.collection_id))) &&
            (!selectedGemTypes.length || selectedGemTypes.includes(String(product.gem_id))) &&
            (!selectedCategories.length || selectedCategories.includes(String(product.category_id)))
        );
    });
...
function updateProductContainer(products) {
    ...
    products.forEach(product => {
        ...
        const productImage = product.image ? `/img/${product.image}` : 'path_to_default_image.jpg'; // Replace with your default image path
